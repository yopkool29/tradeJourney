    ...
    try {
        ...
        if (!userId || !dbName) {
            throw createAppError({
                ...
                tag: 'api.backup.list.no_database'
            })
        }
        ...
        try {
            ...
            const backupPromises = files
                ...
                .map(async f => {
                    try {
                        // Format attendu: backup-{dbName}-{appVersion}-YYYY-MM-DD-HH-MM-{uuid}.zip
                        const dateMatch = f.match(/^backup-(.+)-(\d{4})-(\d{2})-(\d{2})-(\d{2})-(\d{2})-([a-f0-9]+)\.zip$/)
