...
export type RMultipleReliability =
    | 'reliable'
    | 'partial'
    | 'approximate'
    | 'none'

export const getRMultipleReliability = (
    trades: RMultipleTrade[]
): RMultipleReliability => {
...
export const getPLRatioInR = (
    ...
    const avgWin =
        winningRs.length > 0
            ? winningRs.reduce((acc, r) => acc + r, 0) / winningRs.length
            : 0
    const avgLoss =
        losingRs.length > 0
            ? Math.abs(
                  losingRs.reduce((acc, r) => acc + r, 0) / losingRs.length
              )
            : 0
...
export const getTotalProfitLossInR = (
    ...
    return {
        totalProfit: _round(totalProfit, round),
        totalLoss: _round(totalLoss, round),
    }
}
