<template>
	<div class="space-y-4">
		...
		<div>
			...
			<UIInput v-model="params.symbol" size="sm" />
		</div>

		<div>
			<label class="block text-sm font-medium mb-2">Commission %</label>
			<UIInput v-model="params.commissionPercent" type="number" size="sm" />
...
const convert = async () => {
	...
	try {
		...
		formData.append('commissionPercent', String(params.commissionPercent))
