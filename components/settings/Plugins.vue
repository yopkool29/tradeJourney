		...
		<template #header>
			<div class="flex items-center justify-between">
				<div class="header-layout">
					<span class="section-title">{{ $t('pages.settings.plugins.title') }}</span>
				</div>
				<div class="flex items-center gap-2">
					<input ref="fileInput" type="file" accept=".zip" class="hidden" @change="handleFileUpload" />
					<UButton icon="i-heroicons-arrow-up-tray" color="primary" variant="soft" size="sm"
						:loading="importing" @click="fileInput?.click()">
						{{ $t('pages.settings.plugins.import') }}
					</UButton>
					<UButton icon="i-heroicons-arrow-path" color="neutral" variant="ghost" size="sm" :loading="loading"
						@click="refresh">
