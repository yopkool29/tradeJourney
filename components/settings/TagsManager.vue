	...
	<div>
		<div class="header-layout mb-4">
			...
			<CommonModalDefault v-model:open="showAddGroup" :title="$t('components.settings.tags.add_group_modal')">
				...
				<template #content>
					<UForm id="createGroupForm" :state="newGroupState" :validate-on="['change', 'input']" :schema="CreateTagGroupSchema" @submit.prevent="onSubmitGroup">
				...
				<template #footer>
					<div class="action-buttons-end">
						<UButton type="submit" form="createGroupForm" :disabled="!newGroupState.name">{{ $t('common.actions.save') }}</UButton>
						<UButton type="button" variant="soft" @click.prevent="() => { showAddGroup = false }">{{ $t('common.actions.cancel') }}</UButton>
					</div>
				</template>
			</CommonModalDefault>
		</div>
		...
		<div class="flex flex-col gap-6">
			<UCard v-for="group in tagGroups" :key="group.id" class="p-4">
				<div class="flex items-center justify-between mb-3">
					<div class="form-row-lg">
						...
						<CommonModalDefault :open="editGroupStateId === group.id" :title="$t('components.settings.tags.edit_group')" @update:open="(open) => { if (!open) editGroupStateId = null }">
							...
							<template #content>
								<UForm id="modifyGroupForm" :state="newGroupState" :schema="UpdateTagGroupSchema" :validate-on="['change', 'input']" @submit="onSubmitGroup">
									<CommonAlertBox :success-str="successStr" :error-str="errorStr" />
									<UFormField name="name" :label="$t('components.settings.tags.group_name_label')" class="mb-3">
										<UInput v-model="newGroupState.name" :placeholder="$t('components.settings.tags.group_name_placeholder')" autofocus />
									</UFormField>
						...
						<CommonModalDefault :open="groupTagToAdd?.id == group.id" :title="$t('components.settings.tags.add_tag')" @update:open="(open) => { if (!open) groupTagToAdd = null }">
							<template #trigger>
								<UButton icon="i-lucide-plus" size="xs" color="primary" variant="soft" @click="() => { newAddTag(group) }">
									{{ $t('components.settings.tags.add_tag') }}
								</UButton>
							</template>
							<template #content>
								<UForm id="createTagForm" :state="newTagState" :validate-on="['change', 'input']" :schema="CreateTagSchema" @submit.prevent="onSubmitTag">
									<CommonAlertBox :success-str="successStr" :error-str="errorStr" />
									<UFormField name="name" :label="$t('components.settings.tags.tag_name_label')" class="mb-3">
										<UInput v-model="newTagState.name" :placeholder="$t('components.settings.tags.tag_name_placeholder')" autofocus />
									</UFormField>
									<UFormField name="description" :label="$t('components.settings.tags.tag_description_label')" class="mb-3">
										<UInput v-model="newTagState.description" :placeholder="$t('components.settings.tags.tag_description_placeholder')" />
									</UFormField>
								...
								<div class="modal-content-gap">
									<UCheckbox v-model="newTagState.dark_fg_reverse" :label="$t('components.settings.tags.tag_dark_fg_reverse_label')" :disabled="!isDark" class="opacity-100" />
							...
							<template #footer>
								<div class="action-buttons-end">
									<UButton type="submit" form="createTagForm" :disabled="!newTagState.name">{{ $t('common.actions.save') }}</UButton>
				...
				<div class="flex flex-col gap-2 mt-6 w-fit">
					<div v-for="tag in group.tags" :key="tag.id" class="px-4 py-2 text-sm flex items-center gap-4 rounded-lg" :title="tag.description">
						<CommonModalDefault :open="editTagStateId === tag.id" :title="$t('components.settings.tags.edit_tag')" @update:open="(open) => { if (!open) editTagStateId = null }">
							<template #trigger>
							...
							<template #content>
								<UForm id="modifyTagForm" :state="newTagState" :validate-on="['change', 'input']" :schema="UpdateTagSchema" @submit.prevent="onSubmitTag">
									<CommonAlertBox :success-str="successStr" :error-str="errorStr" />
									<UFormField name="name" :label="$t('components.settings.tags.tag_name_label')" class="mb-3">
										<UInput v-model="newTagState.name" :placeholder="$t('components.settings.tags.tag_name_placeholder')" autofocus />
									</UFormField>
									...
									<div class="modal-content-gap">
										...
										<div class="action-buttons">
											<div class="text-md">{{ $t('components.settings.tags.result') }}:</div>
											<UTooltip class="badge-clickable" :text="newTagState.description || newTagState.name">
												<UBadge title="" :label="newTagState.name" :style="getTagStyle(newTagState)" />
											</UTooltip>
										</div>
									</div>
						...
						<UTooltip class="badge-clickable" :text="tag.description || tag.name" @click="editTag(group, tag)">
							<UBadge title="" :label="tag.name" :style="{ backgroundColor: tag.color != '' ? tag.color : '#333', ...(tag.dark_fg_reverse ? { color: '#fff' } : {}) }" />
						</UTooltip>
					</div>
				</div>
			</UCard>
