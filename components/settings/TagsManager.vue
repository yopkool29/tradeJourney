			...
			<UCard v-for="group in tagGroups" :key="group.id" class="p-4">
				<div class="flex items-center justify-between mb-3">
					<div class="form-row-lg">
						...
						<CommonModalDefault :ui="{ overlay: 'z-[300]', content: 'z-[301]' }" :open="editGroupStateId === group.id"
						...
						<CommonModalDelete :ui="{ overlay: 'z-[300]', content: 'z-[301]' }" :open="groupToDelete?.id === group.id" @confirm="onDeleteGroup(group)"
							...
							<template #footer>
								<div class="action-buttons-end">
									<UButton type="submit" form="createTagForm" :disabled="!newTagState.name">{{
										$t('common.actions.save') }}</UButton>
									<UButton variant="soft" @click="groupTagToAdd = null">{{ $t('common.actions.cancel')
										}}</UButton>
								</div>
							</template>
						</CommonModalDefault>
				...
				<div class="flex flex-col gap-2 mt-6 w-fit">
					<div v-for="tag in group.tags" :key="tag.id"
						...
						<CommonModalDefault :ui="{ overlay: 'z-[300]', content: 'z-[301]' }" :open="editTagStateId === tag.id"
						...
						<CommonModalDelete :ui="{ overlay: 'z-[300]', content: 'z-[301]' }" :open="tagToDelete?.id === tag.id" @confirm="onDeleteTag(group, tag)"
							@opened="deleteAssoc = false">
							<template #trigger>
							...
							<template #content>
								<div class="flex flex-col gap-2">
									<p>{{ $t('components.settings.tags.confirm_delete_tag', { name: tagToDelete?.name })
										}}</p>
									<UCheckbox v-model="deleteAssoc"
										:label="$t('components.settings.tags.delete_associations')" />
								</div>
