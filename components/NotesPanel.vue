<template>
    ...
    >
        <div v-if="isOpen" class="fixed left-0 top-0 h-full bg-white dark:bg-gray-800 shadow-lg z-50 flex" style="width: 800px; max-width: 90vw">
                ...
                <div class="p-3 border-b border-gray-200 dark:border-gray-700">
                    ...
                    <div class="space-y-1">
                        <div
                            ...
                            :key="formatDateToYYYYMMDD(item.date)"
                            class="px-2 py-1.5 text-sm rounded-md cursor-pointer transition-colors flex justify-between items-center select-none"
                            :class="{
                                'bg-primary-50 dark:bg-primary-900 text-primary-700 dark:text-primary-200': isActiveDate(
                            ...
                            @click="selectNoteDate(formatDateToYYYYMMDD(item.date))"
                        >
                            <span>{{ formatDateLongString(item.date, locale) }}</span>
                            <UButton
...
const isActiveDate = (dateKey: string) => {
    return formatDateToYYYYMMDD(props.selectedDate) === dateKey
}
...
const saveNote = async () => {
    ...
    try {
        const dateKey = formatDateToYYYYMMDD(props.selectedDate)
...
const deleteNoteConfirmed = async () => {
    ...
    try {
        ...
        if (success) {
            ...
            noteDates.value = noteDates.value.filter((item) => formatDateToYYYYMMDD(item.date) !== formatDateToYYYYMMDD(noteToDelete.value!.date))

            // Si la note supprimée est celle actuellement affichée, réinitialiser l'éditeur
            if (formatDateToYYYYMMDD(props.selectedDate) === formatDateToYYYYMMDD(noteToDelete.value.date)) {
