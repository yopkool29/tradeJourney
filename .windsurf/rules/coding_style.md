...
In TypeScript files prefer `import` to `require` so that we can benefit from type-checking. If it does not work, you may have to add the type using `npm install @types/NAME_OF_PACKAGE`. If you are trying to import an old package, it may not have TypeScript types and in this case using `require()` is acceptable.
 ...
 * `PascalCase.ts`: Only if the file contains a single class, which is the default export.
 * `types.ts` or `fooTypes.ts`: Shared type definitions
...
Doing this avoids having to deal with the `this` keyword.
...
We need to be careful with this and make sure we correctly escape user content.
...
const html = `<a href="${encode(attributes)}">${encode(content)}</a>`;
```
...
Use appropriate escape functions for markdown:

- Escape special characters in table cells
- Escape backticks in inline code
- Escape brackets and parentheses in links
...
Booleans are not a distinct types in many common DBMS, including SQLite and MySQL, so prefer using a `tinyint(1)` instead.
...
These guides may be helpful as references:
