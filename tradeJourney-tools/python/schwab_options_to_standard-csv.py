            ...
            if exec_time_str:
                ...
                if current_trade:
                    try:
                        ...
                        current_trade.legs.append(OptionLeg(
                            strike=strike,
                            option_type=option_type,
                            qty=qty,
                            price=price,
...
def match_trades_with_cash(
    trades: List[OptionTrade],
    cash_transactions: List[CashTransaction]
) -> List[Tuple[OptionTrade, Optional[CashTransaction]]]:
    """
...
def pair_open_close_trades(
    matched_trades: List[Tuple[OptionTrade, Optional[CashTransaction]]]
) -> List[Dict]:
    """
    ...
    for trade, cash in matched_trades:
        ...
        metadata = {
            'spreadType': trade.spread_type,
            'posEffect': trade.pos_effect,
...
def convert_to_standard_format(
    trades: List[Dict],
    account_name: str = "Schwab",
    account_fullname: str = "Charles Schwab Options",
    import_name: str = "SchwabOptions"
) -> List[Dict]:
    """
...
def write_standard_csv(trades: List[Dict], output_path: str):
    ...
    fieldnames = [
        ...
        'instrumentType', 'strikePrice', 'expirationDate', 'optionType', 'premium', 'metadata'
    ]
...
def main():
    parser = argparse.ArgumentParser(
        description='Convert a Schwab Options Account Statement CSV to TradeJourney standard format'
    )
