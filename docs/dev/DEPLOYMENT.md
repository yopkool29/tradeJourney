...
docker exec -it <container_name> npx tsx --require ./scripts/paths-register.cjs scripts/backup-all.ts 1 /path/to/backup/folder

# Restore all databases from a mounted volume
docker exec -it <container_name> npx tsx --require ./scripts/paths-register.cjs scripts/restore-all.ts 1 /path/to/backup/folder
```
